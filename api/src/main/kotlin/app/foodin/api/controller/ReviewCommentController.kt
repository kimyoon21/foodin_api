...
class ReviewCommentController(
    ...
    @PutMapping(value = ["/{id}"])
    fun update(@PathVariable id: Long, @RequestBody updateReq: CommentUpdateReq): ResponseTypeResult<ReviewComment> {
        return ResponseTypeResult(reviewCommentService.update(id, updateReq))
    }

    @PostMapping(consumes = ["application/json"])
    fun register(@RequestBody commentCreateReq: CommentCreateReq): ResponseTypeResult<ReviewComment> {
