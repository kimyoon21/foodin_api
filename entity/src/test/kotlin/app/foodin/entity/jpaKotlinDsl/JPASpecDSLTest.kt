package app.foodin.entity.jpaKotlinDsl
...
@SpringBootTest(classes = [JPASpecificationDSLTest::class])
...
class JPASpecificationDSLTest {
    ...
    fun `Get a tv show by id`() {
        val show = tvShowRepo.findById(hemlockGrove.id).get()
        assertThat(show, equalTo(hemlockGrove))
    }

    @Test
    fun `Get a tv show by id equality`() {
        val show = tvShowRepo.findOne(TvShow::id.equal(theWalkingDead.id)).get()
